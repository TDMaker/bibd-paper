...
int demonstrate_digest(BIO *input, unsigned char *rdresult)
{
    ...
    result = 1;
    memcpy(rdresult, digest_value, digest_length);
...
int get_sha256(const char *filename, unsigned char rdresult[32])
{
    int result = 1;
    FILE *fp = fopen(filename, "r");
    BIO *input = BIO_new_fd(fileno(fp), 1);

    if (input != NULL)
    {
        result = demonstrate_digest(input, rdresult);
        BIO_free(input);
    }
    return result;
}